...
import type { EstadoPedido, PedidoResumen, ResumenAdmin, Restaurante } from '@/types'
...
export function DashboardPanel({
  ...
  return (
    <main className="p-4 md:p-6 max-w-6xl mx-auto">
      <header className="mb-6 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">
            /{restaurante.slug} · Resumen del día
          </p>
        </div>
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4 mb-8">
        <TarjetaResumen
        ...
        />
        <TarjetaResumen
          titulo="Tiempo promedio mesa"
          valor={
            resumen.tiempoPromedioMesa !== null
              ? `${resumen.tiempoPromedioMesa} min`
              : '—'
          }
          subtitulo="Mesas cerradas hoy"
        />
      </section>
              ...
              {ultimosPedidos.length === 0 ? (
                ...
                ultimosPedidos.map((pedido) => (
                  <tr key={pedido.id} className="border-t border-zinc-800">
                    <td className="p-3 font-medium">{pedido.mesaNumero ?? '?'}</td>
