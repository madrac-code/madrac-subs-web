...
export default function DashboardMenuPage() {
  ...
  async function handleToggle(item: MenuItem) {
    ...
    const { data, error } = await supabase
      ...
      .single()
    setAccionId(null)
    if (error || !data) { mostrarFeedback('error', 'Error al actualizar'); return }
    actualizarItemEnLista(data as MenuItem)
    mostrarFeedback('ok', `${data.nombre} ${data.disponible ? 'activado' : 'desactivado'}`)
  }
  ...
  async function handleGuardar(item: MenuItem, values: MenuItemFormValues) {
    ...
    const { data, error } = await supabase
      ...
      .single()
    setAccionId(null)
    if (error || !data) { mostrarFeedback('error', 'Error al guardar'); return }
    actualizarItemEnLista(data as MenuItem)
    setEditandoId(null)
  ...
  async function handleCrear(values: MenuItemFormValues) {
    ...
    const { data, error } = await supabase
      .from('menu_items')
      .insert({ ...values, restaurante_id: restauranteId })
      .select()
      .single()
    setGuardandoNuevo(false)
    if (error || !data) { mostrarFeedback('error', 'Error al crear'); return }
    actualizarItemEnLista(data as MenuItem)
  ...
  async function handleEliminar(item: MenuItem) {
    ...
    setAccionId(null)
    if (error) { mostrarFeedback('error', 'Error al eliminar'); return }
  ...
  return (
    <main className="p-4 md:p-6 max-w-2xl mx-auto">
      ...
      {mensaje && (
        <p className={`mb-4 p-3 rounded-lg text-sm text-center ${
          mensaje.tipo === 'ok' ? 'bg-green-950 text-green-400' : 'bg-red-950 text-red-300'
        }`}>
          {mensaje.texto}
        </p>
      )}
      ...
      {!mostrarNuevo ? (
        ...
        <div className="mb-6">
          ...
          <MenuItemForm onSubmit={handleCrear} onCancel={() => setMostrarNuevo(false)}
            guardando={guardandoNuevo} submitLabel="Crear ítem" />
        </div>
      )}
      ...
      {cargando ? (
        ...
        <p className="text-center text-zinc-500 py-8">No hay ítems en el menú todavía</p>
      ) : (
        <div className="space-y-8">
          {categorias.map((cat) => (
            <section key={cat}>
              <h2 className="text-xl font-semibold border-b border-zinc-800 pb-2 mb-3">{cat}</h2>
                    ...
                    return (
                      <div key={item.id}>
                        <MenuItemForm initial={item}
                          ...
                          onCancel={() => setEditandoId(null)} guardando={procesando} />
                      </div>
                    )
                  }
                  return (
                      ...
                      <div className="flex justify-between items-start gap-3">
                        ...
                        <button type="button" onClick={() => handleToggle(item)}
                          ...
                          className={`shrink-0 px-3 py-1 rounded-full text-xs font-semibold disabled:opacity-50 ${
                            item.disponible ? 'bg-green-500/20 text-green-400' : 'bg-zinc-700 text-zinc-400'}`}>
                          {procesando ? '…' : item.disponible ? 'Activo' : 'Inactivo'}
                        </button>
                      </div>
